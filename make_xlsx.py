...
⚠ 版番号は 2026-09-23 に繰り下げた。**1.0 を提出版に充てるため**。
  ...
  PPTX「投資家向け企画書 Ver1.4.pptx」は未改番（松田さんの指示待ち）。
...
M2D = [
    ...
    (20, "★ 法人の年次解約率", [0.0, 0.30, 0.30, 0.30, 0.30], None,
     "★年30%（松田さん指示）。B2B SaaSの一般水準（5〜15%）より高く、保守側に置いている"),
    ...
    (28, "★ 個人の年次解約率", [0.0, 0.30, 0.30, 0.30, 0.30], None,
     "★年30%（松田さん指示）。個人向けSaaSとしてはむしろ低め。法人と同率で置いている"),
...
title(M3, "③ 越境C2C — 商品別",
      "ToC（個人向け）は**上限＝定価**で成約。ToB（中小企業・企業オーダー）は交渉が入るので**中点**で成約。"
...
for name, grp, seg, hmin, hmax, pmax, cnt, why in S3:
    ...
    put(M3, r, 9, '=IF($C%d="ToC",G%d,H%d)' % (r, r, r), fmt=N, fill=KEYBG, align="right", font=F_B)
...
HIST = [
    ...
    ("0.8 数値改訂", "2026-09-23", "③ 価格を ToC＝上限／ToB＝中点 に分ける",
     "ToC（個人向け6商品＋個人オーダー）は定価で交渉が入らない。ToB（中小企業6商品＋企業オーダー）は"
     "相見積もりと交渉が入るので上限に張り付かない。5期GMV 142.5億→91.8億"),
    ("", "", "② に解約率を新設（★法人・個人とも 年30%）",
     "契約数は「新規獲得−解約」の純増に。法人600→481社、個人17,000→14,180人。②売上 41.6億→34.3億"),
    ("", "", "②の解約が①にも波及",
    ...
    ("", "", "再計算の結果",
     "5期 売上 120.0→94.9億、営業利益 45.4→26.4億（37.8%→27.8%）。黒字化は3期のまま。"
     "2期末キャッシュ198百万でシリーズA前の資金ショートなし"),
]
r = 4
...
put(HX, r, 1, "⚠ 0.4 → 0.5 は構造の作り直し、0.5 → 0.6 は前提を実勢データで詰めたもの、"
             ...
             "5期の売上は 88.1億 → 130.3億 → 120.0億、社員は 61名 → 22名。"
             "数字が良くなった分だけ、前提の説明責任が重くなっている。", font=F_R, border=False)
put(HX, r + 1, 1, "⚠ PPTX「投資家向け企画書 Ver1.4.pptx」はまだ旧番号のまま。"
                  "改番するかは松田さんの指示待ち。", font=F_R, border=False)
