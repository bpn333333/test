...
for i, (nm, cp, seats) in enumerate([("Enterprise", m2CPE, 10), ("Standard", m2CPS, 5)]):
    rr = 12 + i
...
ROLES = [
    ...
    ("管理・コーポレート", 550, [1, 1, 1, 2, 3], "AX化で極小。取引処理はBPO、専門領域は社外の顧問。1期から1名（設立・調達・契約の実務）"),
]
r = 4
...
put(H, 45, 1, "⚠ 管理・コーポレート3名は「作業量が3人分」ではなく「独立性と職務分掌が成立する最小構成」。"
             "記帳・支払・消込はBPOと自動化に出す前提で、人数はAXで減らせる。"
             "減らせないのは下表の3つ（独立性・分掌・責任者）。内訳は73行目以降。", font=F_R, border=False)
...
band(H, 73, "管理・コーポレート3名の内訳と職務分掌")
put(H, 74, 1, "「3名では薄い」と言われたときに出す表。人数ではなく、誰が何を担い、誰が承認するかで答える。"
             "作業量はAXとBPOで圧縮できるが、独立性・職務分掌・責任者はできない。",
    font=F_S, border=False)
header(H, 75, ["担当", "人数", "担う業務", "", "", "", "", "承認・チェックは誰が／外にいる専門家"])
ADMIN = [
    ("経理・決算・開示", 1, "月次・四半期・年次決算、開示書類、税務、資金繰り、外貨と為替予約",
     "承認は CFO（経営・CxO 3名に含む）。記帳・支払・消込はBPO。税務は税理士"),
    ("法務・コンプライアンス", 1, "業務委託契約（制作28名・開発19名・クリエイター681名稼働）、"
     "資金決済法、中国パートナー、知財の帰属", "顧問弁護士（5期30百万）。契約審査は外、運用は社内"),
    ("内部監査（専任・独立）", 1, "J-SOXの運用評価、委託先の往査、③の資金受払（年95,200件）の統制テスト",
     "★ここは兼任できない。統制を運用する人と評価する人は別人でなければならない"),
]
for i2, (nm, n2, work, who) in enumerate(ADMIN):
    rr = 76 + i2
    put(H, rr, 1, nm, font=F_B)
    put(H, rr, 2, n2, fmt=N, fill=INBG, align="right")
    put(H, rr, 3, work, font=F_S)
    put(H, rr, 8, who, font=F_R if nm.startswith("内部監査") else F_S)
put(H, 79, 1, "計", font=F_B)
put(H, 79, 2, "=SUM(B76:B78)", fmt=N, fill=KEYBG, align="right", font=F_B)
put(H, 79, 3, "実働は2名。3名のうち1名は内部監査に取られる", font=F_S)
put(H, 81, 1, "⚠ AXで消せないもの — 人数を増やしても能力を上げても解けない種類の制約",
    font=F_R, border=False)
ADMIN_NG = [
    ("内部監査の独立性", "1人が10人分の処理能力を持っていても、自分で自分を監査できない。評価者は運用者と別人"),
    ("職務分掌（起票と承認の分離）", "管理が1名だと支払の起票も承認も同じ人になる。能力ではなく構造の欠陥として指摘される"),
    ("内部統制報告書の責任者", "署名する人が社内に要る。外注できない"),
]
for i2, (nm, why) in enumerate(ADMIN_NG):
    rr = 82 + i2
    put(H, rr, 1, nm, font=F_B, border=False)
    put(H, rr, 3, why, font=F_S, border=False)
put(H, 86, 1, "⚠ 逆に、3名で回る根拠はBPOと外部専門家。その費用が上の47行目以降で見えていること。"
             "BPO（5期10億）に記帳・支払・消込が含まれる前提だが、シート上のラベルは「CS・運用」のまま。★内訳の明示は未了",
    font=F_R, border=False)
...
HIST = [
    ...
    ("", "", "1期の社員を5名→6名（松田さん指示）",
     "管理・コーポレートを1期から1名置く。設立・調達・業務委託契約の実務が初年度から発生するため。"
     "1期人件費 44.1→50.5百万、1期末キャッシュ 45→37百万（月次バーン950万で4ヶ月分）"),
    ("", "", "②Standard の席数を 3→5（松田さん指示）",
     "席単価24万は据え置き。Standard年額 196→244万で1席48.8万となり、Enterprise 56.4万/席との逆転が解消。"
     "5期②売上 37.5→39.4億"),
    ("", "", "管理・コーポレート3名の内訳と職務分掌を明記（数値は変更なし）",
     "経理1・法務1・内部監査1（専任・独立）。承認はCFO。作業量はAXとBPOで圧縮できるが、"
     "独立性・職務分掌・内部統制報告書の責任者は圧縮できない。「3名では薄い」への回答をシートに持たせた"),
    ("", "", "再計算の結果",
     "5期 売上 120.0→117.4億、営業利益 45.4→41.9億（37.8%→35.7%）。減少分は②の解約と追加費用。"
     "黒字化は3期のまま。2期末キャッシュ209百万でシリーズA前の資金ショートなし"),
]
r = 4
...
CMP = [
    ...
    ("　② ツール外販", 4158, "p2", "★法人5%・個人30%の解約率を新設／Standardを5席に"),
