...
CAP = [
    ("J-KISS バリュエーションキャップ", 6.0, "億円", "確定", "ポストマネー・キャップ"),
    ("シード調達額", 1.42, "億円", "確定", "目標1.5億のうち0.08億は代表個人が直接出資"),
    ("転換時のシード持分", None, "%", "計算", "＝ 調達額 ÷（キャップ ＋ 調達額）"),
    ("希薄化係数 d", 0.758, "倍", "［仮置き］", "シリーズA3億・ESOP10%・IPO公募20%をまとめたもの"),
    ("上場後のシード持分", None, "%", "計算", "CAP_TABLE.md 第5章"),
    ("出資 → 上場", 4.9, "年", "計画", "2026年10月出資 → 2031年9月 東証グロース"),
]
r = 4
for name, v, unit, kind, note in CAP:
    put(C, r, 1, name)
    if name.startswith("転換時"):
        put(C, r, 2, "=B5/(B4+B5)", fmt="0.0%", fill=CALCBG, align="right", font=F_B)
    elif name.startswith("上場後"):
        put(C, r, 2, "=B6*B7", fmt="0.0%", fill=CALCBG, align="right", font=F_B)
    else:
        put(C, r, 2, v, fmt="0.00", fill=INBG, align="right")
    put(C, r, 3, unit, align="center")
    put(C, r, 4, kind, font=F_R if kind == "［仮置き］" else F_N, align="center")
...
SRC = [
    ...
    ("希薄化係数 d", "0.758", "［仮置き］",
     "シリーズA3億・ESOP10%・IPO公募20%を一括で置いたもの"),
