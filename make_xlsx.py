# -*- coding: utf-8 -*-
"""事業計画を Excel モデルとして書き出す（現行 Ver1.6）
...
入力は「前提」と各「商品マスタ」の黄色セルだけ。他は全部そこを参照する数式。
  ...
  python make_xlsx.py    ->  事業計画_Ver1.6.xlsx
...
HIST = [
    ("1.4", "2026-09-23", "B案・強気。制作を半減しツールとC2Cを伸ばす",
     "人手に比例する①で無理をして、比例しない②③を小さく置いていたため"),
    ("", "", "市場規模の次に「業界の売上」を新設",
     "実在企業と並べないと88億の位置が読めない"),
    ("1.5", "2026-09-23", "一本値をやめ、事業×商品まで割って積み上げ",
    ...
    ("1.6", "2026-09-23", "③ 工数は旧maxを標準に置き直し",
...
for ver, date, what, why in HIST:
    ...
    r += 1
r += 1
put(HX, r, 1, "⚠ 1.4 → 1.5 は構造の作り直し、1.5 → 1.6 は前提を実勢データで詰めたもの。"
             "5期の売上は 88.1億 → 108.0億、社員は 61名 → 22名。"
