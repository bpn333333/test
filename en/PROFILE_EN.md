# English materials — Noriyoshi Matsuda

作成: 2026-08-11
用途: エキスパートネットワーク（GLG / AlphaSights / Guidepoint / Third Bridge / Dialectica / Coleman・ビザスク）
および AI 評価プラットフォーム（Outlier / Mercor / Alignerr / Mindrift）、LinkedIn

**方針**: 「日本市場 × 暗号資産の事業実務 × 生成AIの社内導入」を1人で持っていることが希少価値。
翻訳者やコンサル志望ではなく、**事業側の当事者**として書く。
...
month) using Claude, and built the internal web application around it.

He works in Japanese (native) and English.
```
...
| 書いたこと | 狙い |
|---|---|
| 「led / designed and drove」 | **「自分の施策で36倍」とは言わない**（PLAN 1.11）。設計と推進を主導した、という事実に留める |
| JVCEA/FSA を明記 | 海外クライアントが最も知りたいのは**日本の規制環境の実務感覚**。ここが最大の差別化 |
| Recruit を「16 years」で出す | 海外では Recruit = Indeed の親会社として通じる。信用の土台 |
| Taiwan 7 years / 17 companies | クロスボーダーの実務。**中国語圏の案件が回ってくる入口** |
| 生成AI を最後に置く | いま最も引きの強いテーマ。ただし主張の核ではなく「実務でやっている」証拠として |

---

## 2. Expertise tags（登録時に選ぶ／自由入力する専門領域）

**優先順に入力する。多く入れすぎると精度が落ちる。**

| # | タグ | なぜ効くか |
|---|---|---|
| 1 | **Japan market entry / Go-to-market strategy (Japan)** | 海外クライアントの最頻出テーマ |
| 2 | **Digital assets / Crypto exchange operations (Japan, JVCEA/FSA)** | **最も競合が少ない。**日本の規制下で事業を回した日本人は希少 |
| 3 | **Web3 / Token economics / NFT commerce** | 同上 |
| 4 | **Japan HR & recruiting industry (Recruit Holdings)** | 業界エキスパートとしての指名が来る |
| 5 | **Enterprise generative AI adoption (Japan)** | 需要が急増中。実測値を持っている |
| 6 | **Cross-border expansion: Japan / Taiwan / China** | 台湾7年 |
| 7 | **Business planning & FP&A** | 年350億円規模の経営企画 |
| 8 | Japanese consumer behavior / SMB digitalization | 補助 |

---

## 3. Stated hourly rate（登録時に自分で決める）

| プラットフォーム種別 | 提示する額 |
|---|---|
| **エキスパートネットワーク（GLG等）** | **JPY 45,000 / hour（約 USD 300）** |
| ビザスク | JPY 40,000 / 1時間 |
| AI評価（Outlier / Mercor 等） | プラットフォーム側の提示に従う（USD 30〜40/h 相当） |

> **下げない。** エキスパートネットワークは「安い専門家」を探していない。
> Director クラスの日本市場エキスパートは USD 300〜500/h が相場帯。
> **最初に低く出すと、その額が記録に残って上げられなくなる。**

---

## 4. English CV（1ページ。PDF化して各所にアップロードする）

```
NORIYOSHI MATSUDA
Tokyo, Japan | flying.knight333@gmail.com
Business planning & go-to-market operator | Japan market, digital assets, cross-border APAC
...
ADDITIONAL
────────────────────────────────────────────────────────────
• Content: 1,400 published articles in Japanese. Approximately 140 were
  Japanese adaptations of English-language financial media articles, delivered
  as an ongoing engagement.
• Languages: Japanese (native), English (business), Mandarin (conversational)
• Tools: Claude / ChatGPT / Gemini, SQL, Redash, Dune Analytics
```

**⚠️ 確認が必要な箇所（本人しか分からない）**
...
## 5. LinkedIn（エキスパートネットワークのリクルーターはここを漁っている）

**PLAN 1.14 のとおり、LinkedIn は「信用のため」ではなく直接の入口。**
リクルーターは `Japan` × `crypto` / `market entry` / `Recruit` で検索している。
**検索に引っかかる語を Headline と About に入れることが目的。**
...
Claude, and built the internal tooling around it.

Areas I am most often asked about:
• Japan market entry and go-to-market
• Digital asset / crypto exchange operations under Japanese regulation
• Japan's HR and recruiting industry
• Cross-border expansion between Japan, Taiwan and China
• Generative AI adoption inside Japanese companies

Open to expert calls and advisory engagements. Based in Tokyo.
```

> **最後の一行「Open to expert calls and advisory engagements」は必ず入れる。**
> エキスパートネットワークのリクルーターはこの文言で検索している。
