...
UNITS = [60, 304, 1213, 2529, 3727]          # sales.py（②の解約が制作会社チャネルに波及）
...
# ── ② ツール外販（据え置き）────────────────────────────
CORP = [0, 15, 99, 314, 578]        # 期末（★解約率5%適用後）
CORP_ACV = [0, 307, 329, 345, 360]   # 積み上げ（2階建て・別売り。Standardは5席）
INDIE = [0, 0, 2000, 7400, 14180]   # 期末（★解約率30%適用後）
INDIE_ACV = 12
...
C2C_BUILD = [6, 74, 786, 2442, 4274]      # 手数料30%・価格は上限（ToC・ToBとも）・③-C件数を120億に調整
C2C_GP = [0.667, 0.787, 0.791, 0.792, 0.792]   # 30%から決済・送金・システム原価を引いた後
OLD_GMV = [50, 400, 1600, 4500, 8300]   # Ver1.2の一本値（参考。ここには寄せない）
CANNIB_GMV = [0, 0, 0, 0, 0]            # c2c.py で処理済み

# ── 社員（据え置き）────────────────────────────────────
BURDEN = 1.16
# org.py：社員はPM中心、開発はオフショア業務委託、管理はAX化で極小
ROLE = {"経営・CxO": 900, "モデル開発PM": 950, "制作統括": 650,
...
OFFSHORE = [5, 9, 14, 16, 19]               # AX後の実人数（必要工数 ÷ AX倍率）
AX_DEV = [1.0, 1.2, 1.5, 1.9, 2.2]          # ★ミドル中心なのでレビューが制約。制作より低く置く
OFF_COST = [26, 47, 74, 84, 100]            # 百万円。年527万（越BSE20%＋越PG80%のピラミッド）
...
row("オフショアエンジニア", OFFSHORE)
row("  エンジニアのAX倍率", AX_DEV, "{:>9.1f}")
