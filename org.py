...
print("=" * 92)
print("参考：世界の最安帯（英語圏向けのグローバル単価）")
print("=" * 92)
print("  ⚠ ここまでの表は**日本向けの受託単価**（オフショア開発.com）。"
      .replace("**", ""))
print("     英語で直接契約する場合のグローバル単価は別で、もっと安い。")
print()
GLOBAL = [
    ("バングラデシュ", 15, 35, "最安帯"),
    ("パキスタン", 15, 30, "最安帯"),
    ("ネパール", 18, 55, "ジュニア18〜24／シニア38〜55"),
    ("エジプト", 20, 35, "アフリカで競争力"),
    ("ナイジェリア", 25, 99, "幅が広い。需要が強く上振れする"),
]
print("  %-14s %14s %16s  %s" % ("国", "時給($)", "月額(万円・160h)", "備考"))
print("  " + "-" * 74)
for nm, lo, hi, note in GLOBAL:
    print("  %-14s %6d〜%-6d %7.0f〜%-7.0f  %s"
          % (nm, lo, hi, lo * 160 * 150 / 10000, hi * 160 * 150 / 10000, note))
print()
print("  → 世界最安は **バングラデシュ・パキスタンの $15/時（月36万円）**。".replace("**", ""))
print("     ベトナムの日本向け50万より28%安い。")
print()
print("  ただし日本向け単価とは別物であることに注意。")
print("  オフショア開発.comではバングラデシュのシニアは 52.5万/月で、ベトナム(50.0)より**高い**。"
      .replace("**", ""))
print("  差は日本語対応・ブリッジSEのコスト。英語で直接回せるなら最安帯に手が届くが、")
print("  その前提は「PMが英語でブリッジを担う」こと。当社はPMを社員に置く設計なので成立しうる。")
print()
print("  ⚠ ただしモデル開発は当社の競争優位そのもの。")
print("    チェックポイントとLoRAが②の商品で、AX倍率の源泉でもある。")
print("    そこを最安の国に出すのは、単価以外のリスク（知財・継続性・品質）を取ることになる。")
print("    最安を追うなら、コア（モデル）と周辺（ツール・運用）を分けるのが筋。★")
print()
...
if OFF_COST[0] > OLD_OFF[0]:
    print("     1期は逆に高くつく（AX倍率が立つ前に単価だけ上がるため）。")
else:
    print("     中国・東欧を外したので、1期から安い。単価を上げても総額は下がる。")
print("     効き方は制作のAXと同じ。人数を減らして単価の高い層に絞る。")
print()
