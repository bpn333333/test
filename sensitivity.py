...
BASE = dict(
    ...
    # ③ 越境C2C
    gmv=17663, take=0.30, c2c_gp=0.793,
    # 費用（5期・百万円）
...
CASES = [
    ("基準（Ver1.6）", {}, ""),
    ("─ ③ の前提を倒す ─", None, ""),
    ("③ 価格を上限→中点", dict(gmv=10245), "成約が価格帯の真ん中に落ちる"),
    ("③ 価格を上限→下限", dict(gmv=2390), "供給が厚ければ競争で下限に寄る"),
    ("③ 件数を半分", dict(gmv=8832), "月9,400件→4,700件"),
    ("─ ① の前提を倒す ─", None, ""),
    ...
    ("─ 複合 ─", None, ""),
    ("保守（③中点・AX3.0・②半分）", dict(gmv=10245, ax=3.0, corp=300, indie=8000), ""),
    ("悲観（③下限・AX2.0・②半分・①2,500本）",
     dict(gmv=2390, ax=2.0, corp=300, indie=8000, units=2500), ""),
]
...
print("結論")
print("=" * 104)
cons = calc(gmv=10245, ax=3.0, corp=300, indie=8000)
pess = calc(gmv=2390, ax=2.0, corp=300, indie=8000, units=2500)
...
print("  %-24s 売上 %6.1f億  営業利益 %6.1f億（%.1f%%）  シード %.1f倍"
      % ("基準（Ver1.6）", B["rev"] / 100, B["op"] / 100, B["opr"] * 100,
...
print("     **Ver1.6は、置ける前提を全部強い側に置いた場合の上限値**に近い。".replace("**", ""))
print("     悲観ケースでも黒字で、シードは %.1f倍。事業として成立はする。"
      % ((pess["p2"] * 6 + pess["p3"] * 4 + pess["p1"] * 1.5) * SHARE / CHECK))
print()
print("  ⚠ 投資家に出すなら、基準値だけでなくこの幅を一緒に出すべき。")
print("     130億という数字を単独で出すと、前提の1つが崩れた時点で全部疑われる。")
