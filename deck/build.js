...
/* ═══════════ 2. エグゼクティブサマリー ═══════════ */
{
  ...
  stat(s, M,           y0, w, h, "社員22名", "5期の体制で、年商117.4億", "売上/社員 5.34億。制作も開発も業務委託と自動化で回す", VERM);
  stat(s, M+w+0.3,     y0, w, h, "92%",     "人手に比例しない収益（5期）", "② ツール外販39.4億 ＋ ③ C2C 42.7億 ＋ ①のAXで比例が切れた分", NAVY);
  ...
  const ex=[["47.1倍","460億（ライン別・保守）"],["57.3倍","560億（ライン別）"],["67.5倍","660億（ライン別・強気）"]];
  ...
  card(s, M+(w2+0.3)*2, y1, w2, h2, "どこへ向かうか",
    ["3期に黒字化、5年目に年商117.4億・営業利益41.9億（35.7%）。",
...
/* ═══════════ 6. 業界の売上 ═══════════ */
{
  ...
  sec(s, M, 5.42, W, "当社の5期 117.4億が立つ位置（億円）");
  const bw2=(W-0.6)/2;
  ...
  hbar(s, M+bw2+0.6, 6.06, bw2, 0.30, 0.246, "当社 5期（うち①制作は35.2）", "117", VERM, 2.9);
...
/* ═══════════ 11. AXで原価が変わる ═══════════ */
{
  ...
  const rows=[
    ...
    [{text:"1.0倍（AXなし）",options:{bold:true}},"31.0万（8時間相当）","55.0万","41.8%","+33.3億",{text:"28.3%",options:{bold:true,color:MUTED}}],
    ["2.0倍","15.5万","39.5万","58.2%","+39.0億","33.3%"],
    [{text:"3.0倍（ここでも成立）",options:{bold:true,color:NAVY}},{text:"10.3万",options:{color:NAVY}},{text:"34.4万",options:{color:NAVY}},{text:"63.6%",options:{color:NAVY}},{text:"+41.0億",options:{color:NAVY}},{text:"37.0%",options:{bold:true,color:NAVY}}],
    [{text:"4.0倍（計画）",options:{bold:true,color:VERM}},{text:"7.8万（2時間相当）",options:{bold:true,color:VERM}},{text:"31.8万",options:{bold:true,color:VERM}},{text:"66.4%",options:{bold:true,color:VERM}},{text:"+41.9億",options:{bold:true,color:VERM}},{text:"35.7%",options:{bold:true,color:VERM}}],
  ];
  ...
  warn(s, M, 4.66, (W-0.3)/2, 1.12, "★ 4.0倍は現時点の仮置きです。実測して更新します",
    ["1期の最初の20案件で、着手から検収までのディレクション実工数を案件ごとに記録。",
     "2027年6月までに実測値へ差し替えます。AXなし（1倍）でも営業利益率28.3%で成立する構造です。"]);
  warn(s, M+(W-0.3)/2+0.3, 4.66, (W-0.3)/2, 1.12, "浮いた分は研究開発に戻します",
    ["5期の研究開発費は16億、売上の13.6%。モデルが強くなる→AX倍率が上がる→原価が下がる→さらに戻せる。",
...
{
  const s = base(false);
  head(s, 3, "ユニットエコノミクス", "売上117.4億が何の積み上げなのかを、1本・1契約・1取引あたりで示します");
  ...
  const rowsB=[
    ...
    [{text:"制作会社（法人）",options:{bold:true,color:GOLD}},"360万","578社",{text:"20.8億",options:{bold:true,color:GOLD}}],
    ...
    [{text:"合計",options:{bold:true}},"","",{text:"39.4億",options:{bold:true,color:GOLD}}],
  ...
  sec(s, M+6.3, 4.18, 5.73, "5期 売上117.4億の内訳");
  hbar(s, M+6.3, 4.56, 5.73, 0.34, 1.00, "③ C2C手数料", "42.7億", NAVY, 2.4);
  hbar(s, M+6.3, 4.94, 5.73, 0.34, 0.92, "② ツール外販", "39.4億", GOLD, 2.4);
...
/* ═══════════ 19. 知財・法規制・リスク ═══════════ */
{
  ...
  table(s, M, 4.28, W, [
    hrow(["項目","対応","いつ"]),
    [{text:"★ 資金決済法上の位置づけ",options:{bold:true,color:VERM}},{text:"③の仲介が収納代行で済むか、資金移動業の登録が必要か。登録要否で事業設計が変わる最重要論点",options:{color:VERM}},{text:"★設立前に弁護士と確認",options:{bold:true,color:VERM}}],
    ["データの越境","中国のデータ越境規制と、日本の個人情報保護法への対応（越境移転の同意・委託先管理）","1年目"],
    ["業務委託契約","偽装請負と読まれない設計（指揮命令をしない／成果物単位で発注する）","設立前に雛形を作成"],
  ...
  foot(s, "★は仮置きです。資金決済法の該当性と業務委託の契約設計は③を作る前（設立前）に、保険の料率と填補範囲・責任上限の水準は1年目に確定させます");
}
...
/* ═══════════ 20. チーム ═══════════ */
{
  ...
  const rows=[
    hrow(["","1期","2期","3期","4期","5期"]),
    [{text:"社員（日本側）",options:{bold:true,color:VERM}},{text:"6",options:{bold:true}},"9","15","20",{text:"22",options:{bold:true,color:VERM}}],
    ["　うちモデル開発PM","1","2","3","4","4"],
    ["　うち経営・統括・管理","5","7","12","16","18"],
    ...
    [{text:"★ 売上／社員",options:{bold:true}},"750万","3,958万","1.61億","3.38億",{text:"5.34億",options:{bold:true,color:VERM}}],
  ];
...
/* ═══════════ 21. 事業計画 ═══════════ */
{
  ...
  s.addChart(p.ChartType.bar, [
    { name:"売上高", labels, values:[45,356,2420,6756,11738] },
    { name:"営業利益", labels, values:[-109,-130,521,2282,4195] },
  ], {
  ...
  card(s, cx, 2.74, cw, 0.9, "3期 ― ツールが立ち上がる",
    ["ツール外販5.9億・C2C7.9億。ここで黒字化します（＋5.2億・21.5%）。"], GOLD, 9.5);
  card(s, cx, 3.76, cw, 1.14, "4〜5期 ― 回収",
    ["3期に黒字化。5期に年商117.4億・営業利益41.9億（35.7%）。社員は22名にとどめます。"], NAVY, 9.5);
  const rows=[
    hrow(["","① 制作（本数）","② ツール ／ ③ C2C","ARR（②）","期末現金","人手に比例しない"]),
    ["1期","39（60本）","— ／ 6","—","37","13%"],
    ["2期","230（304本）","52 ／ 74","46","209","57%"],
    ["3期（N-2期）","1,040（1,213本）","594 ／ 786","560","805","80%"],
    ["4期（N-1期）","2,266（2,529本）","2,048 ／ 2,442","1,946","2,952","89%"],
    [{text:"5期（N期）→ 2031年 上場",options:{bold:true,color:VERM}},{text:"3,522（3,727本）",options:{bold:true}},{text:"3,941 ／ 4,274",options:{bold:true}},{text:"3,746",options:{bold:true,color:GOLD}},{text:"6,643",options:{bold:true,color:NAVY}},{text:"92%",options:{bold:true,color:VERM}}],
  ];
  table(s, M, 5.06, W, rows, [2.3,2.5,2.6,1.5,1.5,1.63], 9, 0.3);
...
/* ═══════════ 22. 上場までの流れ ═══════════ */
{
  ...
  warn(s, M, 4.96, W, 1.28, "ゴールは上場ではありません",
    ["グロースは2030年3月以降「上場5年経過後に時価総額100億円」の維持基準が新設されました。2031年上場なら2036年に100億円。",
     "当社の想定時価総額は、保守的なライン別評価でも460億。維持基準100億に対して4.6倍のバッファがあります。"]);
...
{
  const s = base(false);
  head(s, 8, "出口とリターン", "5期の売上117.4億・営業利益41.9億を前提に試算しています");
  const y0=1.58, h=1.46, w=(W-0.6)/3;
  stat(s, M,           y0, w, h, "460〜660億", "ライン別に倍率を分けた場合", "ツール6〜8倍＋C2C4〜6倍＋制作1.5〜2.5倍。証券会社が実際にやる見方", VERM);
  stat(s, M+w+0.3,     y0, w, h, "470〜587億", "全社にPSR4〜5倍", "人手に比例しない収益が92%であることを根拠にする", NAVY);
  stat(s, M+(w+0.3)*2, y0, w, h, "5.34億", "売上／社員（5期）", "社員22名で年商117.4億。国内SaaSの平均2,000〜4,000万に対して桁が違う", GOLD);
  sec(s, M, 3.20, W, "シード投資家のリターン — 上場後持分 14.5%（ESOP10%・シリーズA・IPO公募20%で希薄化後）");
  const rows=[
    hrow(["時価総額","根拠","シードの取り分","倍率"]),
    ["460億","ツール6倍＋C2C4倍＋制作1.5倍（保守）","66.9億",{text:"47.1倍",options:{bold:true,color:VERM}}],
    ["560億","ツール7倍＋C2C5倍＋制作2.0倍","81.4億",{text:"57.3倍",options:{bold:true,color:VERM}}],
    ["660億","ツール8倍＋C2C6倍＋制作2.5倍（強気）","95.9億",{text:"67.5倍",options:{bold:true,color:VERM}}],
  ];
  table(s, M, 3.52, W, rows, [2.0,5.4,2.4,2.23], 10, 0.40);
  warn(s, M, 5.04, (W-0.3)/2, 1.20, "維持基準は大きく超えます",
    ["グロースの「上場5年経過後に時価総額100億円」に対し、保守的な460億でも4.6倍のバッファ。",
     "上場時の創業者持分は42%前後、評価額は約194億（460億のとき）を想定しています。"]);
  warn(s, M+(W-0.3)/2+0.3, 5.04, (W-0.3)/2, 1.20, "すべて「計画が達成された場合」の試算です",
    ["②のツール外販39.4億は海外展開が前提で、Synthesia 210億の19%にあたる規模です。",
...
/* ═══════════ 24. マイルストーン ═══════════ */
{
  ...
  const rows=[
    ...
    ["3期","制作1,213本・黒字化・ツールの海外展開に着手","AX 2.2倍／ツール5.9億／通期黒字／監査開始"],
    ["4期","制作2,529本・スケール","AX 3.0倍／ツール20.5億／C2C 24.4億／流通株式25%の設計合意"],
    [{text:"5期",options:{bold:true,color:VERM}},{text:"制作3,727本・上場申請",options:{bold:true}},{text:"AX 4.0倍／年商117.4億・営業利益41.9億",options:{bold:true}}],
  ];
  ...
  warn(s, M, 4.98, W, 1.26, "最優先で実測するのは ③の成約価格です",
    ["★ ToC・ToBとも上限成約で置いています。実際にどこで成約するかを1期から件数と単価の両方で実測します。",
     "AX倍率は次点です。★20案件で実測し2027年6月に更新。4倍なら35.7%、1倍でも28.3%で成立します。"]);
...
/* ═══════════ 25. 資金計画 ═══════════ */
{
  ...
  sec(s, M, 1.52, 6.3, "1年目の使途（単位 万円・合計 11,750万）");
  const items=[["人件費（社員6名）",5050,VERM],["研究開発（GPU・モデル・データ基盤）",3000,GOLD],
  ...
  items.forEach(function(it,i){ hbar(s, M, 1.90+i*0.40, 6.3, 0.34, it[1]/5050, it[0], String(it[1]), it[2], 3.3); });
  ...
  warn(s, M+6.9, 3.52, w2, 1.52, "使途の4分の1が研究開発です",
    ["1年目に使うのは 1億1,750万。残り約3,250万は2期への持ち越しです。",
     "最大の費目は人件費（社員6名・5,050万）、次が研究開発（3,000万）。",
...
/* ═══════════ 26. リスクと対策 ═══════════ */
{
  ...
  const rows=[
    ...
    [{text:"1",options:{bold:true}},{text:"高",options:{bold:true,color:VERM}},{text:"高",options:{bold:true,color:VERM}},{text:"②のツール外販が海外で売れない",options:{bold:true}},"5期39.4億の半分は海外。国内だけなら20億前後に落ちる。1期からSynthesia・HeyGenの価格と機能を追い、勝てる領域（日本語・日本市場の要求）に絞る"],
  ...
  warn(s, M, 5.42, W, 0.82, "①の規模を追わないぶん、リスクは②に寄っています",
    ["制作を売上の30%に抑えた代わりに、②39.4億と③42.7億が計画の70%になりました。ここが最大の賭けです。"]);
