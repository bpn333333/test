...
/* ═══════════ 2. エグゼクティブサマリー ═══════════ */
{
  ...
  const ex=[["12.4倍","時価総額121億（PSR6倍）"],["14.4倍","141億（PSR7倍）※中心値"],["16.5倍","161億（PSR8倍）"]];
  ex.forEach(function(e,i){
    const x=M+2.7+i*3.05;
    s.addText(e[0], { x, y:4.06, w:1.2, h:0.42, fontFace:F, fontSize:19, bold:true, color:VERM, valign:"middle", margin:0, isTextBox:true });
  ...
  s.addText("シード1.42億 → 上場後持分14.5%（ESOP10%・シリーズA・IPO公募20%で希薄化後）。出資から約4.9年。",
    { x:M+2.7, y:4.06, w:0, h:0, fontFace:F, fontSize:1, color:PAPER, margin:0, isTextBox:true });
  s.addText("シード1.42億 → 上場後持分14.5%（ESOP10%・シリーズA・IPO公募20%で希薄化後）。出資から約4.9年",
    { x:M+0.26, y:4.42, w:2.4, h:0.44, fontFace:F, fontSize:8, color:MUTED, valign:"top", lineSpacingMultiple:1.2, margin:0, isTextBox:true });
  ...
  card(s, M+(w2+0.3)*2, y1, w2, h2, "どこへ向かうか",
    ["1〜3年目は投資期間。4年目に通期黒字化、5年目に年商20億・営業利益6.1億。",
     "売上の76%を「人手に比例しない収益」にする。ここが上場審査で効く。"], GOLD, 8.5, SOFT2);
...
{
  const s = base(false);
  head(s, 3, "ユニットエコノミクス", "売上20億が何の積み上げなのかを、1本・1契約・1取引あたりで示します");
  s.addText("① 映像制作 — 1本あたり（平均単価50万円のとき）", { x:M, y:1.52, w:6.0, h:0.3, fontFace:F, fontSize:11, bold:true, color:VERM, margin:0, isTextBox:true });
  const rowsA=[
    hrow(["","金額","備考"]),
  ...
  hbar(s, M+6.3, 4.50, 5.73, 0.34, 1.00, "② ツール外販", "10.5億", GOLD, 2.2);
  hbar(s, M+6.3, 4.88, 5.73, 0.34, 0.46, "③ C2C手数料", "4.9億", NAVY, 2.2);
  hbar(s, M+6.3, 5.26, 5.73, 0.34, 0.46, "① 映像制作", "4.8億", VERM, 2.2);
  warn(s, M+6.3, 5.62, 5.73, 0.62, "②③で76%。だからSaaS寄りで見ています",
    ["①は人手に比例しますが、②③は契約数・件数が増えても人員が比例して増えません。"]);
  foot(s, "制作の単価・原価は BUSINESS_PLAN 7-1。ツール外販の単価と契約数、C2Cの手数料率・平均単価・件数は、すべて未実測の［仮置き］です");
}
...
/* ═══════════ 11. 収益モデル ═══════════ */
{
  ...
  const rows=[
    ...
    [{text:"①",options:{bold:true,color:VERM}},{text:"AI映像制作の受注 → クリエイターへ配分",options:{bold:true}},"フロー",{text:"初日から",options:{bold:true}},{text:"4.8億",options:{bold:true,color:VERM}},"粗利率60%。事業の主体。ここでデータが溜まる"],
  ...
  warn(s, M+6.3, 3.56, 5.73, 1.42, "順番が大事です",
    ["①の制作を数百本こなさないと、②で売れるチェックポイントもLoRAも育ちません。",
     "制作が先、ツールが後。この順序だから、他社が同じものを作るには同じ年数がかかります。"]);
  warn(s, M+6.3, 5.06, 5.73, 1.18, "③は②の副産物です",
    ["自動化したツールを外部の発注者に開放したものが③。新たに作るものはほとんどありません。"]);
...
/* ═══════════ 18. 事業計画 ═══════════ */
{
  ...
  card(s, cx, 1.6, cw, 1.02, "1期 ― 基盤をつくる",
    ["映像制作54本で2,700万。C2C発注PFは6ヶ月目に公開。営業利益 ▲3,200万は計画通りの投資です。"], VERM, 9.5);
  card(s, cx, 2.74, cw, 0.9, "2〜3期 ― 投資期間",
    ["制作体制の拡大とツール外販の立ち上げ。赤字は2期に最大化（▲6,700万）し、3期はほぼ均衡まで戻ります。"], GOLD, 9.5);
  card(s, cx, 3.76, cw, 1.14, "4〜5期 ― 回収",
    ["4期に通期黒字化。5期に年商20億・営業利益6.1億（30.4%）。時価総額121〜161億の水準へ。"], NAVY, 9.5);
  const rows=[
    hrow(["","売上の内訳（百万円）① 制作 ／ ② ツール ／ ③ C2C","この期の到達点"]),
    ["1期","① 27 ／ ③ 9","制作54本・PF公開・登録100名"],
    ["2期","① 105 ／ ② 20 ／ ③ 54","ツール外販の初期契約／シリーズA 3億"],
    ["3期（N-2期）","① 200 ／ ② 160 ／ ③ 162","監査開始。赤字がほぼ均衡まで戻る"],
    ["4期（N-1期）","① 320 ／ ② 504 ／ ③ 324","通期黒字化。内部管理体制の構築"],
    [{text:"5期（N期）→ 2031年 上場",options:{bold:true,color:VERM}},{text:"① 480 ／ ② 1,052 ／ ③ 486",options:{bold:true}},{text:"売上20億・営業利益6.1億",options:{bold:true}}],
  ];
  table(s, M, 5.06, W, rows, [3.0,4.5,4.53], 10, 0.3);
  foot(s, "計画値。ツール外販の契約数、C2Cの手数料率・発注件数はすべて未実測の［仮置き］です。1期に実測して差し替えます。数字は plan_c2c.py が計算しています");
}
