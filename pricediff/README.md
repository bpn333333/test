...
「元 × 為替」の単純比較ではなく、**重さから国際送料を積み上げた「日本着原価」と比べる**のが要点です。
差額300円に見えた商品が、1.2kg あって送料で消える——という取り違えを防ぐためです。

```
 #            商品名             原価    Amazon    楽天    価格差    差率   重さ   Amazonランク 楽天ランク
--- -------------------------- -------- -------- -------- --------- ------ ------- ------------ ----------
  1 トラベルポーチ 6点セット      1,453    2,980    2,680    +1,527  +105%    410g     22,400位       圏外
  2 デスク収納ラック 木製         3,128    4,280    3,980    +1,152   +37%  1.25kg     45,100位       圏外
  3 ステンレス保温タンブラー …    1,566    2,480    2,180      +914   +58%    320g      8,420位       14位
  4 LEDセンサーライト 人感 US…    1,126    1,980    1,780      +854   +76%    180g      5,630位        7位
```
...
価格差 = 日本側の最高値(Amazon と楽天の高い方) − 日本着原価
差率   = 価格差 ÷ 日本着原価
```

- **課金重量**は実重量と容積重量(縦×横×高さ÷6000)の重い方。かさばる商品の見落としを防ぎます
- 重量が取れない行は `default_weight_g`(既定500g)で計算し、`重さ取得元` に「既定値」と記録します
- 為替は `buffer_pct`(既定2%)だけ円安側に倒して計算します。取らぬ狸を数えないため
- 数値はすべて `config.yaml` で自分の代行業者の実績値に置き換えてください。**既定値は概算です**

> 価格差 = 利益ではありません。ここから販売手数料(Amazon 8〜15%)・FBA配送料・
> 国内送料・広告費が引かれます。まず「送料込みでも差がある候補」を絞るための道具です。
