...
title(M3, "③ 越境C2C — 商品別",
      "下限はクリエイターの留保価格（工数×時給÷(1−手数料)）。上限は日本の発注者の支払意思。"
      "両側から挟まれた中で価格が決まる。★上限・工数・件数は提案値。")
header(M3, 3, ["商品", "分類", "工数min", "工数max", "下限（供給）", "上限（需要）", "中点",
...
for name, grp, hmin, hmax, pmax, cnt, why in S3:
    ...
    put(M3, r, 5, "=ROUNDUP(クリエイター経済!$B$10*C%d/(1-$B$26)/500,0)*500" % r,
    ...
    put(M3, r, 9, "=(C%d+D%d)/2*H%d" % (r, r, r), fmt=N, align="right")
...
ROLES = [
    ...
    ("モデル開発エンジニア", 850, [2, 6, 16, 28, 40], "AX倍率を上げ続ける中核"),
...
line("rnd", "研究開発費", lambda i, c, col: "=%s" % pf(pRND, i),
     note="AX倍率を上げ続けるための投資")
