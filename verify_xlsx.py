...
print("    転換時のシード持分  %.1f%%   （＝1.42億÷6億。ポストマネー・キャップ）"
      % (cell("資本構成の推移", "C15") * 100))

fr = find("資本構成の推移", "流通株式比率")
flo = cell("資本構成の推移", "E%d" % fr)
print()
print("    流通株式の内訳")
for lab in ["創業者（役員）", "ESOP枠", "シード投資家 保有分", "シリーズA 保有分", "公募・売出し"]:
    rr = find("資本構成の推移", lab)
    print("      %-18s 保有 %5.1f%%   算入 %5.1f%%"
          % (lab, cell("資本構成の推移", "B%d" % rr) * 100,
             cell("資本構成の推移", "E%d" % rr) * 100))
print("      %-18s          %5.1f%%   基準25%% に対して %+.1fポイント  %s"
      % ("流通株式比率", flo * 100, (flo - 0.25) * 100,
         "満たす" if flo >= 0.25 else "不足"))
if flo < 0.25:
    OK[0] = False
