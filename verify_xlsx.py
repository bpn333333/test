# -*- coding: utf-8 -*-
"""事業計画_Ver0.8.xlsx を実際に再計算し、plan_v15.py と突き合わせる
...
check("商品マスタ①", "受注本数（営業体制シート）", [60, 304, 1213, 2529, 3727], 4, I5, "%10.0f")
check("商品マスタ①", "① 売上（百万円）", [39, 230, 1040, 2266, 3522], 8, I5, "%10.0f")

print()
print("=" * 96)
print("営業体制 — チャネルの積み上げ")
print("=" * 96)
check("営業体制", "受注本数 合計", [60, 304, 1213, 2529, 3727], 4, None, "%10.0f")
check("営業体制", "取引アカウント数", [15, 55, 175, 319, 453], 4, None, "%10.0f")

print()
print("=" * 96)
print("商品マスタ②③")
print("=" * 96)
check("商品マスタ②", "積み上げACV（万円）", [0, 307, 329, 345, 360], 2, None, "%10.0f")
check("商品マスタ②", "② 売上 合計（百万円）", [0, 52, 594, 2048, 3941], 6, None, "%10.0f")
check("商品マスタ③", "GMV 合計", [19, 248, 2621, 8140, 14248], 40, None, "%10.0f")
check("商品マスタ③", "③ 売上（手数料30%）", [6, 74, 786, 2442, 4274], 15, None, "%10.0f")
check("商品マスタ③", "売上総利益", [4, 59, 623, 1936, 3389], 15, None, "%10.0f")

print()
print("=" * 96)
print("損益計算書 — plan_v15.py と突き合わせ")
print("=" * 96)
check("損益計算書", "① 映像制作", [39, 230, 1040, 2266, 3522], 8, None, "%10.0f")
check("損益計算書", "② ツール外販", [0, 52, 594, 2048, 3941], 6, None, "%10.0f")
check("損益計算書", "③ C2C手数料", [6, 74, 786, 2442, 4274], 15, None, "%10.0f")
check("損益計算書", "売上高", [45, 356, 2420, 6756, 11738], 60, None, "%10.0f")
check("損益計算書", "売上総利益", [18, 213, 1705, 5001, 8880], 60, None, "%10.0f")
check("損益計算書", "人件費（社員）", [50, 77, 129, 172, 187], 2, None, "%10.0f")
check("損益計算書", "開発委託費（オフショア）", [26, 47, 74, 84, 100], 3, None, "%10.0f")
check("損益計算書", "獲得費", [8, 62, 241, 593, 1002], 8, None, "%10.0f")
check("損益計算書", "営業利益", [-109, -130, 521, 2282, 4195], 60, None, "%10.0f")
check("損益計算書", "社員数", [6, 9, 15, 20, 22], 0.5, None, "%10.0f")
check("損益計算書", "顧客が浮かせる額", [39, 230, 1040, 2266, 3522], 8, None, "%10.0f")
