...
check("商品マスタ①", "受注本数（営業体制シート）", [60, 304, 1213, 2517, 3679], 3, I5, "%10.0f")
check("商品マスタ①", "① 売上（百万円）", [39, 230, 1040, 2255, 3477], 6, I5, "%10.0f")

print()
print("=" * 96)
print("営業体制 — チャネルの積み上げ")
print("=" * 96)
check("営業体制", "受注本数 合計", [60, 304, 1213, 2517, 3679], 3, None, "%10.0f")
check("営業体制", "取引アカウント数", [15, 55, 175, 318, 447], 3, None, "%10.0f")
...
check("商品マスタ②", "② 売上 合計（百万円）", [0, 46, 556, 1893, 3578], 5, None, "%10.0f")
check("商品マスタ③", "GMV 合計", [13, 163, 1571, 4790, 8336], 40, None, "%10.0f")
check("商品マスタ③", "③ 売上（手数料30%）", [4, 49, 471, 1437, 2501], 15, None, "%10.0f")
check("商品マスタ③", "売上総利益", [3, 39, 373, 1138, 1981], 15, None, "%10.0f")

print()
print("=" * 96)
print("損益計算書 — plan_v15.py と突き合わせ")
print("=" * 96)
check("損益計算書", "① 映像制作", [39, 230, 1040, 2255, 3477], 6, None, "%10.0f")
check("損益計算書", "② ツール外販", [0, 46, 556, 1893, 3578], 5, None, "%10.0f")
check("損益計算書", "③ C2C手数料", [4, 49, 471, 1437, 2501], 15, None, "%10.0f")
check("損益計算書", "売上高", [43, 325, 2067, 5586, 9555], 60, None, "%10.0f")
check("損益計算書", "売上総利益", [16, 188, 1426, 4075, 7165], 60, None, "%10.0f")
check("損益計算書", "人件費（社員）", [44, 77, 129, 172, 187], 2, None, "%10.0f")
check("損益計算書", "開発委託費（オフショア）", [26, 47, 74, 84, 100], 3, None, "%10.0f")
check("損益計算書", "獲得費", [8, 61, 236, 569, 945], 6, None, "%10.0f")
check("損益計算書", "営業利益", [-102, -154, 264, 1437, 2633], 60, None, "%10.0f")
check("損益計算書", "社員数", [5, 9, 15, 20, 22], 0.5, None, "%10.0f")
check("損益計算書", "顧客が浮かせる額", [39, 230, 1040, 2255, 3477], 6, None, "%10.0f")
