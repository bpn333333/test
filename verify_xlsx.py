...
check("商品マスタ①", "受注本数（営業体制シート）", [60, 304, 1208, 2464, 3494], 4, I5, "%10.0f")
check("商品マスタ①", "① 売上（百万円）", [39, 230, 1035, 2208, 3302], 8, I5, "%10.0f")

print()
print("=" * 96)
print("営業体制 — チャネルの積み上げ")
print("=" * 96)
check("営業体制", "受注本数 合計", [60, 304, 1208, 2464, 3494], 4, None, "%10.0f")
check("営業体制", "取引アカウント数", [15, 55, 174, 311, 424], 4, None, "%10.0f")
...
check("商品マスタ②", "② 売上 合計（百万円）", [0, 46, 546, 1851, 3430], 6, None, "%10.0f")
...
print("=" * 96)
check("損益計算書", "① 映像制作", [39, 230, 1035, 2208, 3302], 8, None, "%10.0f")
check("損益計算書", "② ツール外販", [0, 46, 546, 1851, 3430], 6, None, "%10.0f")
check("損益計算書", "③ C2C手数料", [6, 61, 535, 1596, 2754], 15, None, "%10.0f")
check("損益計算書", "売上高", [45, 336, 2116, 5655, 9486], 60, None, "%10.0f")
check("損益計算書", "売上総利益", [18, 197, 1464, 4134, 7117], 60, None, "%10.0f")
check("損益計算書", "人件費（社員）", [44, 77, 129, 172, 187], 2, None, "%10.0f")
check("損益計算書", "開発委託費（オフショア）", [26, 47, 74, 84, 100], 3, None, "%10.0f")
check("損益計算書", "獲得費", [8, 61, 234, 560, 913], 8, None, "%10.0f")
check("損益計算書", "営業利益", [-101, -146, 303, 1505, 2640], 60, None, "%10.0f")
check("損益計算書", "社員数", [5, 9, 15, 20, 22], 0.5, None, "%10.0f")
check("損益計算書", "顧客が浮かせる額", [39, 230, 1035, 2208, 3302], 8, None, "%10.0f")
